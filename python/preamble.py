...
class _Node:
    def __init__(self, ntype, start=0):
        ...
        self._keyframes = {}          # {int_t: {prop: val}}
        self._children = []
    ...
    def _set(self, key, val):
        ...
        self._keyframes.setdefault(self._t, {})[key] = val
        return self

    def _resolve(self, key):
        """Last value of key in keyframes with t strictly less than self._t."""
        val = None
        for t in sorted(k for k in self._keyframes if k < self._t):
            if key in self._keyframes[t]:
                val = self._keyframes[t][key]
        return val
...
# ── group ──────────────────────────────────────────────────────────────────
class _Group:
    """Forwards method calls to every node in the group, silently skipping
    nodes that don't support the operation."""
...
def _make(ntype, at, kw):
    ...
    for k, v in kw.items(): n._set(k, v)
    return _add(n)

def node(at=0, **kw):   return _make('node',   at, kw)
def rect(at=0, **kw):   return _make('rect',   at, kw)
def circle(at=0, **kw): return _make('circle', at, kw)
def group(nodes):        return _Group(nodes)
