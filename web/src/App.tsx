...
interface RenderedFrameResponse {
  n: number;
  png: string;
}
...
function hitTest(
  ...
  for (const node of nodes) {
    ...
    if (
      clickX >= b.x - tol &&
      clickX <= b.x + b.width + tol &&
      clickY >= b.y - tol &&
      clickY <= b.y + b.height + tol
    ) {
      if (node.kind === "group") {
...
export default function App() {
  ...
  // ── sequence ──────────────────────────────────────────────────────────────
  const [seqRenderResults, setSeqRenderResults] = useState<Map<string, SequenceRenderResult>>(
    new Map(),
  );
  ...
  useEffect(() => {
    ...
    const fetchTree = async (n: number): Promise<SceneData | null> => {
      ...
      const r = await fetch(withToken(`/tree/${n}?v=${runId}${sceneParam}`), {
        signal: ctrl.signal,
      });
      if (!r.ok) return null;
    ...
    Promise.all([fetchTree(frame), frame > 0 ? fetchTree(frame - 1) : Promise.resolve(null)])
      .then(([curr, prev]) => {
        setSceneData(curr);
  ...
  useEffect(() => {
    ...
    const ctrl = new AbortController();
    fetch(withToken(`/frame/${frame}?scale=${sk}&v=${runId}${sceneParam}`), {
      signal: ctrl.signal,
    })
  ...
  useEffect(() => {
    ...
    const ctrl = new AbortController();
    fetch(withToken(`/node-bounds?frame=${frame}${sceneParam}`), {
      signal: ctrl.signal,
    })
  ...
  useEffect(() => {
    ...
    if (!el || sceneWidth == null || sceneHeight == null || sceneWidth <= 0 || sceneHeight <= 0) {
  ...
  const imgSrc =
    imageCacheRef.current.get(cacheKey(frame, scaleKey)) ??
    ...
    return (
      <div className="tl-frame-slider">
        ...
        >
          ...
          <div
            className="tl-ov-window"
            style={{ left: `${ovWinLeft}%`, width: `${ovWinWidth}%` }}
          />
          ...
          {Array.from({ length: winSize }, (_, i) => {
            ...
            return (
              <div key={fi} className="timeline-tick-wrap" style={{ left: `${pct}%` }}>
                ...
                {showNum && <div className={numClass}>{fi}</div>}
              </div>
            );
          })}
          <input
      ...
      <div className="ws-overlay">
        <div className="ws-error-box">
          ...
          <div className="ws-error-body">
            Invalid or missing token. Open the URL printed by the server.
          </div>
        </div>
      </div>
    );
    ...
    <>
      <div className="app">
        {wsStatus === "reconnecting" && (
          <div className="ws-banner ws-banner-reconnecting">Connection lost — reconnecting…</div>
        )}
        {wsStatus === "failed" && (
          <div className="ws-overlay">
            <div className="ws-error-box">
              ...
              <div className="ws-error-body">
                Could not reconnect to the server. Restart the server and reload the page.
              </div>
              <button className="ws-error-reload" onClick={() => window.location.reload()}>
                Reload
              </button>
            </div>
          </div>
        ...
        <div className="app-body">
          ...
          <PanelGroup orientation="horizontal" className="main-content">
                ...
                <Panel defaultSize={20} minSize={10}>
                  <div className="console-panel">
                    <div className="console-header">
                      ...
                      {running && (
                        <button className="console-stop-btn" onClick={terminate}>
                          ■ Stop
                        </button>
                      )}
                    </div>
            ...
            <Panel defaultSize={50} minSize={20}>
              <div
                className={`right-column${!isFfsqActive && running ? " right-column-evaluating" : ""}${isFfsqActive ? " right-column-seq" : ""}`}
              >
                {isFfsqActive ? (
                  /* ── Sequence player ── */
                  <SequencePlayer
                    result={currentSeqResult}
                    fps={fps}
                    canvasAreaRef={seqCanvasAreaRef}
                    onRequestRerender={handleSeqRerender}
                    isRendering={running}
                  />
                ) : (
                  <>
                      ...
                      {hasScene && (
                        <>
                          ...
                          <div className="tl-controls">
                            ...
                            <button
                              className="tl-btn"
                              onClick={() => setFrame(prevCueFrame!)}
                              disabled={prevCueFrame === null || isActive}
                              title="Prev cue frame"
                            >
                              ◂●
                            </button>
                            <button
                              className="tl-btn"
                              onClick={() => setFrame(nextCueFrame!)}
                              disabled={nextCueFrame === null || isActive}
                              title="Next cue frame"
                            >
                              ●▸
                            </button>
                          ...
                          <div ref={canvasContentRef} className="canvas-content">
                            {hasScene && canvasLayout && (
                              <div
                                ...
                                style={{
                                  width: canvasLayout.cssWidth,
                                  height: canvasLayout.cssHeight,
                                  cursor: "crosshair",
                                }}
                                onClick={(e) => {
                                  if (!sceneData || sceneWidth == null || sceneHeight == null)
                                    return;
                                  const rect = e.currentTarget.getBoundingClientRect();
                                  const sceneX =
                                    ((e.clientX - rect.left) * sceneWidth) / canvasLayout.cssWidth;
                                  const sceneY =
                                    ((e.clientY - rect.top) * sceneHeight) / canvasLayout.cssHeight;
                                  const tol = (5 * sceneWidth) / canvasLayout.cssWidth;
                                  setSelectedNid(
                                    hitTest(sceneData.children, sceneX, sceneY, allNodeBounds, tol),
                                  );
                                }}
                              >
                                <img
      ...
      {exportDialogOpen && (
        <div
          ...
          onClick={() => {
            if (exportStatus !== "exporting") setExportDialogOpen(false);
          }}
        >
          <div className="export-dialog" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="export-dialog-body">
              ...
              <div className="export-row">
                ...
                <select
                  value={exportCodec}
                  onChange={(e) => handleCodecChange(e.target.value as typeof exportCodec)}
                >
