...
pub struct ExportSeqVideoParams {
    ...
    #[serde(default = "default_crf")] pub crf: u32,
}
...
async fn do_export_seq_video(state: AppState, params: ExportSeqVideoParams, tx: UnboundedSender<String>) {
    // Validate filename
    let filename = params.filename.trim().to_string();
    if filename.is_empty() || filename.contains('/') || filename.contains('\\') || filename.contains("..") {
        send(&tx, serde_json::json!({"type":"error","message":"Invalid filename"}));
        return;
    }
    ...
    let ffmpeg_ok = tokio::process::Command::new("ffmpeg")
        ...
        .unwrap_or(false);
    if !ffmpeg_ok {
        send(&tx, serde_json::json!({"type":"error","message":"ffmpeg is not installed or not in PATH. Please install ffmpeg to use video export."}));
        return;
    }
    ...
    let seq_content = match tokio::fs::read_to_string(&params.seq_path).await {
        Ok(c) => c,
        Err(e) => {
            send(&tx, serde_json::json!({"type":"error","message":format!("Failed to read sequence: {e}")}));
            return;
        }
    };
    let seq_data: serde_json::Value = match serde_json::from_str(&seq_content) {
        Ok(v) => v,
        Err(e) => {
            send(&tx, serde_json::json!({"type":"error","message":format!("Failed to parse sequence: {e}")}));
            return;
        }
    };
    let scene_files: Vec<String> = match seq_data.get("scene_files").and_then(|v| v.as_array()) {
        ...
        None => {
            send(&tx, serde_json::json!({"type":"error","message":"Sequence has no scene_files"}));
            return;
        }
    };
    if scene_files.is_empty() {
        send(&tx, serde_json::json!({"type":"error","message":"Sequence has no scene files"}));
        return;
    }
    ...
    let prologue = {
        ...
        cfg.prologue.as_deref().and_then(|p| std::env::current_dir().ok().map(|d| d.join(p)))
    };
    ...
    for (i, scene_file) in scene_files.iter().enumerate() {
        ...
        if let Err(e) = crate::package::run_python_to_json(scene_file, &prologue, fps, &json_path).await {
            let _ = tokio::fs::remove_dir_all(&temp_base).await;
            send(&tx, serde_json::json!({"type":"error","message":format!("Failed to process '{}': {e}", scene_file)}));
            return;
        }
    ...
    let render_result = tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        ...
        let total_frames: u64 = anims.iter()
        ...
        for anim in &anims {
            ...
            frames.par_iter().try_for_each(|&frame_n| -> Result<(), String> {
                let scene = anim.build_scene(FrameId::new(frame_n), SceneSelection::All)
                    .map_err(|e| e.to_string())?;
                let pixmap = match target_res {
                    Some((w, h)) => renderer_skia::render_scene_fitted(&scene, w, h),
                    None => renderer_skia::render_scene(&scene, 1.0),
                ...
                std::fs::write(fd.join(format!("frame{file_idx}.png")), &png)
                    .map_err(|e| e.to_string())?;
                let n = done.fetch_add(1, Ordering::Relaxed) + 1;
                let _ = tx3.send(serde_json::json!({"type":"progress","done":n,"total":total}).to_string());
                Ok(())
